...
public class CardPaymentMgmtServiceImpl implements CardPaymentMgmtService
{
    ...
    public List<Map<String, Object>> selectTransInfoList(Map<String, Object> objMap) throws Exception{
    	...
    	String flg = (String )objMap.get( "flag" );
		if(objMap.get( "flagChk" ).equals( "BuyerNm" ) || objMap.get( "flagChk" ).equals( "CardNo" ))
		{
			if(objMap.get( "flagChk" ).equals( "CardNo" ) && flg !=""
					&& flg.length() > 11 )
			{
				log.info( "인증 유형 = cardNo -> 암호화 셋팅 " );
				String ispTxtEnc1 = SHA256Util.encrypt( flg.substring( 0,8 ) + "0000" + flg.substring( 12 ) );
				String ispTxtEnc2 = SHA256Util.encrypt( flg.substring( 0,12 ) + "0000");
				objMap.put( "ispText",  ispTxtEnc1);
				objMap.put( "ispText2",  ispTxtEnc1);
				log.info( "암호화 : 1 - " + ispTxtEnc1 + ", 2 - " +ispTxtEnc2 );
			}
			objMap.put( "flag", SHA256Util.encrypt(flg) );
		}
		else
		{
			objMap.put( "flag", flg );
    ...
    @Override
    public Map<String, Object> selectCardInfoTotAmt(Map<String, Object> objMap) throws Exception{
    	Map<String, Object> resultMap = new HashMap<String, Object>();
    	resultMap = ( Map< String, Object > ) cardPaymentMgmtDAO.selectCardTotAmt( objMap );

    	log.info( "selectCardInfoAmt data  : " + resultMap );
    	...
    	int  testCcAmt = (( BigDecimal  ) resultMap.get( "TESTCCAMT" )).intValue();
   		...
   		int totTotCnt =  realAppCnt+realCcCnt+testAppCnt+testCcCnt;
   		int totTotAmt =  realAppAmt+realCcAmt+testAppAmt+testCcAmt;
    	int realTotCnt =  realAppCnt+realCcCnt;
    	int realTotAmt=  realAppAmt+realCcAmt;
    	int testTotCnt =  testAppCnt+testCcCnt;
    	int testTotAmt =  testAppAmt+testCcAmt;

    	int totAppCnt =  realAppCnt+testAppCnt;
    	int totAppAmt =  realAppAmt+testAppAmt;
    	int totCcCnt =  realCcCnt+testCcCnt;
    	int totCcAmt=  realCcAmt+testCcAmt;
