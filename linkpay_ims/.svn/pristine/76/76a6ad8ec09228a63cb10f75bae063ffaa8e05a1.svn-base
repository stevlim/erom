...
public class NoticeMgmtController {
     ...
     * Modify History : Just Created.
     ----------------------------------------------------*/
    @RequestMapping(value="/insertNoticeMgmt.do", method=RequestMethod.POST)
    public ModelAndView insertNoticeMgmt(@RequestBody String strJsonParameter, CommonMap commonMap) throws Exception {
        ModelAndView        objMv  = new ModelAndView();
        Map<String, Object> objMap = new HashMap<String, Object>();
        ...
        try {
            if (!CommonUtils.isNullorEmpty(strJsonParameter)) {
                ...
                resultMap = noticeMgmtService.insertNoticeMgmt(objMap);
                resultMap.put( "flg", "insert" );
                objMv.addObject( "data", resultMap );
            ...
            intResultCode    = 9999;
            strResultMessage = CommonMessage.MSG_ERR_EXCEPTION;
            log.debug("insertNoticeMgmt.do exception : " + ex.getMessage());
        } finally {
            if (intResultCode == 0) {
                objMv = CommonUtils.resultSuccess(objMv);
            } else {
    ...
    /**--------------------------------------------------
     * Method Name    : updateNoticeMgmt
     * Description    : 공지사항 수정
     * Author         : ymjo, 2015. 10. 13.
     * Modify History : Just Created.
     ----------------------------------------------------*/
    @RequestMapping(value="/updateNoticeMgmt.do", method=RequestMethod.POST)
    public ModelAndView updateNoticeMgmt(@RequestBody String strJsonParameter, CommonMap commonMap) throws Exception {
        ModelAndView        objMv  = new ModelAndView();
        Map<String, Object> objMap = new HashMap<String, Object>();
        Map<String, Object> resultMap = new HashMap<String, Object>();
        int    intResultCode    = 0;
        ...
        try {
            if (!CommonUtils.isNullorEmpty(strJsonParameter)) {
                ...
                log.info( "objMap : " + objMap );
                resultMap = noticeMgmtService.updateNoticeMgmt(objMap);
            ...
            intResultCode    = 9999;
            strResultMessage = CommonMessage.MSG_ERR_EXCEPTION;
            log.debug("updateNoticeMgmt.do exception : " + ex.getMessage());
        } finally {
            if (intResultCode == 0) {
                objMv = CommonUtils.resultSuccess(objMv);
            } else {
