 ...
 * Modify History : Just Created.
 ------------------------------------------------------------*/
@Controller
@RequestMapping(value="/baseInfoMgmt/baseInfoMgmt")
public class BaseInfoMgmtController {
     ...
     * Modify History : Just Created.
     ----------------------------------------------------*/
    @RequestMapping(value = "/baseInfoMgmt.do")
	   	...
	   	try
	   	{
	           if (!CommonUtils.isNullorEmpty(strJsonParameter))
	           {
	        	   ...
	        	   if(objMap.get( "frDt") != null)
	        	   {
	        		   ...
	        		   fr_dt = fr_dt.replaceAll("(\\d+)-(\\d+)-(\\d+)", "$3$2$1" );
