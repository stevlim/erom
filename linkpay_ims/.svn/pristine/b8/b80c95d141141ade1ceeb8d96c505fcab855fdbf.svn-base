...
public class BaseInfoMgmtServiceImpl implements BaseInfoMgmtService
{
    ...
    public Map<String, Object> baseInfo(Map<String, Object> paramMap) throws Exception 
    {
    	...
    	try
    	{
    		if(paramMap.get( "MER_VAL" ).equals( "2" ))
    		{
    			log.info( "기본정보조회-" +(logCount++) +  "- MID정보조회" );
    			//mid조회
    			...
    			else 
    			{
    				...
    				String bsKindCd= (String)objList.get( 0 ).get( "BS_KIND_CD" );
    				//카테고리(대분류) 조회
    				log.info( "기본정보조회-2- "+(logCount++)+"카테고리(대,소분류)" );
    				if(bsKindCd == null){
    					
    				}else{
    					String[] kindCd = bsKindCd.split( ":" );
	    				cateList = baseInfoRegistrationDAO.selectCateList(kindCd[0]);
	    				if(cateList.size()>0)
	    				{
	    					String bigCate = (String)cateList.get( 0 ).get( "DESC1" );
	    					objList.get( 0 ).put( "BIG_KIND_CD", bigCate );
	    					String smCate = (String)cateList.get( 0 ).get( "DESC2" );
	    					objList.get( 0 ).put( "SM_KIND_CD", smCate );
    			...
    			log.info( "기본정보조회-" +(logCount++) +"-  정산 수수료 조회");
    			//정산 수수료 조회
    			objList = baseInfoMgmtDAO.selectSettleFee(paramMap);
    			if(objList.size() == 0 )
    			{
    				log.info( "기본정보조회-" +(logCount++) +"- 정산수수료 정보 없음" );
    			}
    			else
    			{
    				log.info( "기본정보조회-" +(logCount++) +  "-  정보data : " + ToStringBuilder.reflectionToString( objList ) );
    				resultMap.put( "settleFeeInfo", objList );
    			}
    			log.info( "기본정보조회-" +(logCount++) +"-  정산 서비스내역 조회");
    			//정산 수수료 조회
    			objList = baseInfoMgmtDAO.selectSettlmntService(paramMap);
    			if(objList.size() == 0 )
    			{
    				log.info( "기본정보조회-" +(logCount++) +"- 정산 서비스내역 정보 없음" );
    			}
    			else
    			{
    				log.info( "기본정보조회-" +(logCount++) +  "- 정산 서비스내역 정보data : " + ToStringBuilder.reflectionToString( objList ) );
    				resultMap.put( "settleServiceInfo", objList );
    			}
    			log.info( "기본정보조회-" +(logCount++) +"-  정산 수수료 내역 조회");
    			//정산 수수료 조회
    			paramMap.put("idCd", "1");
    			paramMap.put("SHOP_TYPE", "01");
    			objList = baseInfoMgmtDAO.selectSettleFee(paramMap);
    			if(objList.size() == 0 )
    			{
    				log.info( "기본정보조회-" +(logCount++) +"- 정산 수수료 내역 정보 없음" );
    			}
    			else
    			{
    				log.info( "기본정보조회-" +(logCount++) +  "- 정산 수수료 내역 정보data : " + ToStringBuilder.reflectionToString( objList ) );
    				resultMap.put( "settleFee", objList );
    			}
    			log.info( "기본정보조회-" +(logCount++) +"-  정산 수수료 내역 조회");
    			//정산 수수료default 조회
    			/*objList = baseInfoMgmtDAO.selectCompFee(paramMap);
    			if(objList.size() == 0 )
    			{
    				log.info( "기본정보조회-" +(logCount++) +"- 정산 수수료 내역default 정보 없음" );
    			}
    			else
    			{
    				log.info( "기본정보조회-" +(logCount++) +  "- 정산 수수료 내역default 정보data : " + ToStringBuilder.reflectionToString( objList ) );
    				resultMap.put( "compFee", objList );
    			}*/
    			log.info( "기본정보조회-" +(logCount++) +"-  카드 정보 조회");
    			objList = baseInfoMgmtDAO.selectCardInfo(paramMap);
    			if(objList.size() == 0 )
    			{
    				log.info( "기본정보조회-" +(logCount++) +"- 카드정보 없음" );
    			}
    			else
    			{
    				log.info( "기본정보조회-" +(logCount++) +  "- 카드 정보 data : " + ToStringBuilder.reflectionToString( objList ) );
    				resultMap.put( "cardInfo", objList );
    			}
    			log.info( "기본정보조회-" +(logCount++) +"-  카드 자동결제정보 조회");
    			//카드자동결제 조회
    			objList = baseInfoMgmtDAO.selectCardBillInfo(paramMap);
    			if(objList.size() == 0)
    			{
    				log.info( "기본정보조회-" +(logCount++) +"- 카드자동결제 정보 없음" );
    			}
    			else
    			{
    				log.info( "기본정보조회-" +(logCount++) +"-  카드 자동결제정보 data : " +ToStringBuilder.reflectionToString( objList ) );
    				resultMap.put( "cardBillInfo", objList );
    			}	
    			//정산주기 조회
    			objList = baseInfoMgmtDAO.selectSettleCycle(paramMap);
    			if(objList.size() == 0)
    			{
    				log.info( "기본정보조회-" +(logCount++) +"- 정산주기 정보 없음" );
    			}
    			else
    			{
    				log.info( "기본정보조회-" +(logCount++) +  "- 정산주기 정보 data : " + ToStringBuilder.reflectionToString( objList ) );
    				resultMap.put( "settleCycle", objList );
    			}       			
    			//지불수단 사용 여부 조회
    			objList = baseInfoMgmtDAO.selectPayType(paramMap);
    			if(objList.size() == 0)
    			{
    				log.info( "기본정보조회-" +(logCount++) +"- 지불수단 정보 없음" );
    			}
    			else
    			{
    				log.info( "기본정보조회-" +(logCount++) +  "- 지불수단 정보 data : " + ToStringBuilder.reflectionToString( objList ) );
    				resultMap.put( "payType", objList );
    			}
    			//사용카드사 조회
    			objList = baseInfoMgmtDAO.selectUseCard(paramMap);
    			if(objList.size() == 0)
    			{
    				log.info( "기본정보조회-" +(logCount++) +"- 사용카드사 정보 없음" );
    			}
    			else
    			{
    				log.info( "기본정보조회-" +(logCount++) +  "- 사용카드사 정보 data : " + ToStringBuilder.reflectionToString( objList ) );
    				resultMap.put( "useCard", objList );
    			}
    			//카드 코드 리스트 조회 
    			...
    			if(objList.size() == 0)
    			{
    				log.info( "기본정보조회-" +(logCount++) +"- 해외카드 코드 리스트 정보 없음" );
    			}
    			else
    			{
    				log.info( "기본정보조회-" +(logCount++) +  "-해외 카드 코드 리스트 정보 data : " + ToStringBuilder.reflectionToString( objList ) );
    				resultMap.put( "overCardCd", objList );
    			}
    			paramMap.put( "code2", "pur" );
    			objList = baseInfoMgmtDAO.selectCardCd(paramMap);
    			if(objList.size() == 0)
    			{
    				log.info( "기본정보조회-" +(logCount++) +"- major카드 코드 리스트 정보 없음" );
    			}
    			else
    			{
    				log.info( "기본정보조회-" +(logCount++) +  "- major카드 코드 리스트 정보 data : " + ToStringBuilder.reflectionToString( objList ) );
    				resultMap.put( "majorCardCd", objList );
    			}
    			paramMap.put( "code2", "*" );
    			objList = baseInfoMgmtDAO.selectCardCd(paramMap);
    			if(objList.size() == 0)
    			{
    				log.info( "기본정보조회-" +(logCount++) +"- minor카드 코드 리스트 정보 없음" );
    			}
    			else
    			{
    				log.info( "기본정보조회-" +(logCount++) +  "- minor카드 코드 리스트 정보 data : " + ToStringBuilder.reflectionToString( objList ) );
    				resultMap.put( "minorCardCd", objList );
    			}
    			paramMap.put( "CODE_CL", "0038" );
    			objList = baseInfoMgmtDAO.selectCodeCl(paramMap);
    			if(objList.size() == 0)
    			{
    				log.info( "기본정보조회-" +(logCount++) +"- 정산주기 코드 리스트 정보 없음" );
    			}
    			else
    			{
    				log.info( "기본정보조회-" +(logCount++) +  "-정산주기 코드 리스트 정보 data : " + ToStringBuilder.reflectionToString( objList ) );
    				resultMap.put( "stmtCycleCd", objList );
    			}
    			
    		}
    		else if(paramMap.get( "MER_VAL" ) .equals( "3" ))
    		{
    			log.info( "기본정보조회-" +(logCount++) +  "- GID정보조회" );
    			//GID SELECT
    			objList = baseInfoRegistrationDAO.selectGidInfo( (String)paramMap.get( "MER_ID" ) );
    			if(objList.size() == 0 )
      		   {
      			   log.info( "기본정보조회-SELECT-1-GID 조회값 null " );
	      		   resultMap.put( "resultCd", "9999" );
	 			   return resultMap;
      		   }
      		   else
      		   {
      			  log.info( "기본정보조회-" +(logCount++) +  "- GID 정보 data : " + ToStringBuilder.reflectionToString( objList ) );
      			   resultMap.put( "gidInfo", objList );
      		   }
    			//등록정보 조회
      		   objList = baseInfoMgmtDAO.selectGMid( paramMap );
      		   if(objList.size() == 0 )
      		   {
      			   log.info( "기본정보조회-SELECT-1-GID MID정보 조회값 null " );
      		   }
      		   else
      		   {
      			  log.info( "기본정보조회-" +(logCount++) +  "- GID,MID 정보 data : " + ToStringBuilder.reflectionToString( objList ) );
      			   resultMap.put( "gidMidInfo", objList );
      		   }
      		   //은행 코드 조회
      		   objList = baseInfoMgmtDAO.selectBankCd( paramMap );
	      		if(objList.size() == 0 )
	   		   {
	   			   log.info( "기본정보조회-SELECT-1-GID BANK_CODE 조회값 null " );
	   		   }
	   		   else
	   		   {
	   			  log.info( "기본정보조회-" +(logCount++) +  "- GID BANK_CODE 정보 data : " + ToStringBuilder.reflectionToString( objList ) );
	   			   resultMap.put( "gidBankList", objList );
	   		   }
      		 
    		}
    		else if(paramMap.get( "MER_VAL" ).equals( "4" ))
    		{
    			log.info( "기본정보조회-" +(logCount++) +  "- VID정보조회" );
    			
    			objList = baseInfoRegistrationDAO.selectVidInfo( (String)paramMap.get( "MER_ID" ) );
    			if(objList.size() == 0 )
      		   {
      			   log.info( "기본정보조회-SELECT-1-VID 조회값 null " );
      			   resultMap.put( "resultCd", "9999" );
      			   return resultMap;
      		   }
      		   else
      		   {
      			  log.info( "기본정보조회-" +(logCount++) +  "- VID 정보 data : " + ToStringBuilder.reflectionToString( objList ) );
      			   resultMap.put( "vidInfo", objList );
      		   }
    			//vid 정산 수수료 정보 
    			objList = baseInfoRegistrationDAO.selectVidFeeInfo( paramMap);
     		   if(objList.size() == 0 )
     		   {
     			   log.info( "기본정보조회-SELECT-1-VID 정산수수료 조회값 null " );
     		   }
     		   else
     		   {
     			  log.info( "기본정보조회-" +(logCount++) +  "- 정산수수료 정보 data : " + ToStringBuilder.reflectionToString( objList ) );
     			   resultMap.put( "vidFeeInfo", objList );
     		   }
     		   //등록정보 조회
     		   objList = baseInfoMgmtDAO.selectVMid( paramMap );
     		   if(objList.size() == 0 )
     		   {
     			   log.info( "기본정보조회-SELECT-1-VID MID정보 조회값 null " );
     		   }
     		   else
     		   {
     			  log.info( "기본정보조회-" +(logCount++) +  "- VID,MID 정보 data : " + ToStringBuilder.reflectionToString( objList ) );
     			   resultMap.put( "vidMidInfo", objList );
     		   }
     		   //은행 코드 조회
      		   objList = baseInfoMgmtDAO.selectBankCd( paramMap );
	      		if(objList.size() == 0 )
	   		   {
	   			   log.info( "기본정보조회-SELECT-1-VID BANK_CODE 조회값 null " );
	   		   }
	   		   else
	   		   {
	   			  log.info( "기본정보조회-" +(logCount++) +  "- VID BANK_CODE 정보 data : " + ToStringBuilder.reflectionToString( objList ) );
	   			   resultMap.put( "vidBankList", objList );
	   		   }
    		}
