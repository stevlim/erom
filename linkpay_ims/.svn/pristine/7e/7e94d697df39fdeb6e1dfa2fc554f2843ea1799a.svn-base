 ...
 * IONPay Global Constants
 ------------------------------------------------------------*/
var IONPay = {
	CSRFID                 : "IMSRequestVerificationToken",
...
IONPay.Utils = {
	...
	fnDatePicker : function(strType, intDiff, strID, strBaseID) {
		...
		var objDate  = intDiff == 0 ? new Date() : new Date(intYear, intMonth, intDay);
...
//	    	$("#" + formID + " input[type='text']").val("");
	...
	fnClearShowUpdateForm: function () {
	    el = jQuery("#div_update_frm .grid .tools").parents(".grid").children(".grid-body").not($("[id^='div_']"));

	    jQuery("#div_updatefrm .grid .tools .expand").removeClass().addClass("collapse");

	    IONPay.Utils.fnClearForm();
	    el.slideUp(200).slideDown(200);
	    $("#spn_frm_title").text("Modify");
	    $("#editMode").val("modify");
	    $("#div_update_frm").show(200);
	},
	...
	fnCheckSearchRange: function ($startDT, $endDT, strRange) {
        ...
        if (diffDay > (strRange * 31)) {
            IONPay.Msg.fnAlert(gMessage("IMS_JS_0005") + " " + strRange + gMessage("IMS_JS_0006"));
...
IONPay.Msg = {
    ...
    /**------------------------------------------------------------
	 * Function Name  : fnResetPwdConfirm
	 * Description    : -
	 * Author         : IY Choi, 2017. 03. 31.
	 * Modify History : Just Created.
	 ------------------------------------------------------------*/
    fnResetPwdConfirm: function (strMsg, strName, strEmailMsg, strEmail, callBackFN) {
        $("body").addClass("breakpoint-1024 pace-done modal-open ");
        strCallBackFN = callBackFN;
		
        $("#modalResetPwdConfirm").text(strMsg);
        $("#modalResetPwdNameConfirm").text(strName);
        
        $("#modalResetEmailConfirm").text(strEmailMsg);
        $("#inputEmail").val(strEmail);
        
        $("#btnModalResetPwdConfirm").click();
    },
...
$(document).ready(function () {    
    ...
    try {
        if (typeof isMerchantInfoMgmt != "undefined" && isMerchantInfoMgmt == true) {
    ...
    $("#btnResetPwdConfirmModalTop, #btnResetPwdConfirmModalBottm").on("click", function () {
	...
	$.validator.addMethod("onlyNumeric", function (value, element, param) {
        return this.optional(element) || /^[0-9]+$/.test(value);
    }, "Please input onlyNumber only for Id.");
    ...
    try {
        $("#divSearchDateType2 > button").on("click", function (e) {
            if ($(this).attr("id") == "btnToday") {
                ...
                IONPay.Utils.fnDatePicker("Month", -2, "txtFromDate2", "txtToDate2");
            } else if ($(this).attr("id") == "btn3Month") {
                IONPay.Utils.fnDatePicker("Month", -3, "txtFromDate2", "txtToDate2");
            }
        });
    } catch(ex) { }
    ...
    try {
        $("#divSearchDateType7 > button").on("click", function (e) {
            if ($(this).attr("id") == "btnToday") {
                IONPay.Utils.fnDatePicker("Day", 0, "txtFromDate1", "txtToDate1");
            } else if ($(this).attr("id") == "btn1Week") {
                IONPay.Utils.fnDatePicker("Day", -6, "txtFromDate1", "txtToDate1");
            } else if ($(this).attr("id") == "btn1Month") {
                IONPay.Utils.fnDatePicker("Month", -1, "txtFromDate1", "txtToDate1");
            } else if ($(this).attr("id") == "btn2Month") {
                IONPay.Utils.fnDatePicker("Month", -2, "txtFromDate1", "txtToDate1");
            } else if ($(this).attr("id") == "btn3Month") {
                IONPay.Utils.fnDatePicker("Month", -3, "txtFromDate1", "txtToDate1");
            }
        });
    } catch (ex) { }
